...
class LZ77:
    def __init__(self):
        self.buff_size = 65_535
        self.bytes_cnt_offset = 2
        self.bytes_cnt_len = 2
        self.minimum_sequence_len = 5
        self.data = bytearray()
        self.file_len = 0
    ...
    def findMatching(self, buffer: [int, int], pos: int) -> (int, int):
        ...
        for i in range(self.minimum_sequence_len, min(buff_size + 1, self.file_len - pos)):
            ...
            with open(file_path_write, "wb") as file_write:
                ...
                while pos < self.file_len:
                    ...
                    if offset == 0 and length == 0:
                        ...
                        file_write.write(offset.to_bytes(self.bytes_cnt_offset, byteorder="big"))
                        file_write.write(length.to_bytes(self.bytes_cnt_len, byteorder="big"))
            ...
            with open(file_path_write, "w+b") as file_writer:
                ...
                while True:
                    ...
                    if type_ == 0:
                        ...
                        i = 0
                        while i < length:
                            offset = int.from_bytes(file_reader.read(self.bytes_cnt_offset),
                                                    byteorder="big")
                            substr_length = int.from_bytes(file_reader.read(self.bytes_cnt_len),
                                                           byteorder="big")
